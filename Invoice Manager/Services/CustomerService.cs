...
namespace Invoice_Manager.Services
{
    public class CustomerService : ICustomerService
    {
        ...
        public IEnumerable<ICustomer> GetAll()
        {
            ...
            var customers = scope.Database.Fetch<CustomerDto>(sql).Select(Customer.CreateFromDto);
        ...
        public CustomerSearchResult GetCustomer(CustomerSearchOption options)
        {
            ...
            using (var scope = _scopeProvider.CreateScope())
            {
                ...
                if (options.Text != null) sql = sql.Where<CustomerDto>(x => x.Name == options.Text || x.GSTNumber == options.Text || x.State == options.Text);
